...
class LiveMCPManager:
    ...
    def create_session(
        ...
        enabled_names = self.server_names
        bound_names = list(dict.fromkeys(server_names or enabled_names))
        unknown_names = sorted(set(bound_names) - set(enabled_names))
        if unknown_names:
            raise ValueError(
                f"session server not enabled in suite: {unknown_names}"
            )
        if not bound_names:
            raise ValueError("session must bind at least one enabled server")
        all_profiles = self._state_profiles(state_profiles)
        resolved_profiles = {
            name: all_profiles[name] for name in bound_names
        }
        session_id = f"{self.suite_config.suite_name}_{next(self._session_counter):06d}"
        spec = SessionSpec(
            session_id=session_id,
            suite_name=self.suite_config.suite_name,
            server_names=bound_names,
            seed=seed,
            created_at=datetime.now(timezone.utc).isoformat(),
            max_turns=int(self.suite_config.rollout.get("max_turns", 8)),
            metadata={"state_profiles": resolved_profiles},
        )
        with self._lifecycle_lock:
                ...
                for server_name in spec.server_names:
                    self._request(
                        ...
                        "session/reset",
                        {
                            "session_id": session_id,
                            "seed": seed,
                            "state_profile": resolved_profiles[server_name],
                        },
                    )
        except Exception:
            with self._lifecycle_lock:
    ...
    def reset_session(
        ...
        with self._operation_lock(session_id):
            ...
            resolved_profiles = (
                self._state_profiles(state_profiles)
                if state_profiles is not None
                else self._state_profiles(spec.metadata.get("state_profiles"))
            )
            try:
                for server_name in spec.server_names:
                    self._request(
                ...
                raise
            spec.seed = seed
            spec.metadata["state_profiles"] = resolved_profiles
    ...
    def _state_profiles(
        ...
        requested = {
            ...
            for owner, profile in (overrides or {}).items()
        }
        unknown = sorted(set(requested) - set(self.server_names))
        if unknown:
            raise ValueError(f"state profile owner not in suite: {unknown}")
        explicit_row_profiles = overrides is not None
        return {
            cfg.name: requested.get(
                cfg.name,
                (
                    "baseline"
                    if explicit_row_profiles
                    else str(cfg.session.get("state_profile", "baseline"))
                ),
            )
            for cfg in self.suite_config.servers
            if cfg.enabled
