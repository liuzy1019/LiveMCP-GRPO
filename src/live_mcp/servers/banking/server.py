...
TOOLS = [
    {"name": "list_accounts", "description": "List all accounts for the current customer, optionally filtered by exact account type.", "input_schema": {"type": "object", "properties": {"type": {"type": "string", "enum": ["savings", "checking", "business", "investment", "retirement", "health", "escrow", "foreign"], "description": "Optional exact account type. Omit to list all types."}}, "required": []}, "annotations": {"readonly": True, "mutating": False}},
    ...
    {"name": "get_history", "description": "Get recent transaction history.", "input_schema": {"type": "object", "properties": {"account_id": {"type": "string"}, "limit": {"type": "integer", "minimum": 1}, "start_date": {"type": "string"}, "txn_type": {"type": "string", "enum": ["transfer", "wire_transfer", "deposit", "withdrawal", "bill_pay"], "description": "Optional exact transaction type."}}, "required": ["account_id"]}, "annotations": {"readonly": True, "mutating": False}},
    {"name": "get_statement", "description": "Get monthly statement with opening/closing balance and transaction summary.", "input_schema": {"type": "object", "properties": {"account_id": {"type": "string"}, "year": {"type": "integer", "minimum": 1900}, "month": {"type": "integer", "minimum": 1, "maximum": 12}}, "required": ["account_id", "year", "month"]}, "annotations": {"readonly": True, "mutating": False}},
    ...
    {"name": "schedule_transfer", "description": "Schedule a positive future transfer between two distinct unfrozen accounts. execute_date must be an ISO YYYY-MM-DD date later than the current reference date.", "input_schema": {"type": "object", "properties": {"from_account": {"type": "string"}, "to_account": {"type": "string"}, "amount": {"type": "number", "exclusiveMinimum": 0}, "execute_date": {"type": "string"}}, "required": ["from_account", "to_account", "amount", "execute_date"]}, "annotations": {"mutating": True}},
    {"name": "list_scheduled_transfers", "description": "List existing scheduled transfers, optionally filtered by source account or exact status. Returns scheduled_txn_id values usable by cancel_transfer.", "input_schema": {"type": "object", "properties": {"account_id": {"type": "string"}, "status": {"type": "string", "enum": ["scheduled", "cancelled", "executed"], "description": "Optional exact scheduled-transfer status."}}, "required": []}, "annotations": {"readonly": True, "mutating": False}},
    ...
    {"name": "unfreeze_account", "description": "Unfreeze a previously frozen account using a non-empty authorization code supplied by the user.", "input_schema": {"type": "object", "properties": {"account_id": {"type": "string"}, "authorization_code": {"type": "string", "minLength": 1}}, "required": ["account_id", "authorization_code"]}, "annotations": {"mutating": True, "sensitive_params": ["account_id", "authorization_code"]}},
...
class BankingServer(StatefulToolServer):
    ...
    def schedule_transfer(self, session_id: str, arguments: dict[str, Any]) -> dict[str, Any]:
        state = self._state(session_id); from_aid, to_aid = arguments["from_account"], arguments["to_account"]; amount = float(arguments["amount"])
        if from_aid == to_aid: raise KeyError("source and destination accounts must be distinct")
        from_acct = self._acct(state, from_aid)
        to_acct = self._acct(state, to_aid)
        if from_acct.get("frozen"):
            raise KeyError(f"account frozen: {from_aid}")
        if to_acct.get("frozen"):
            raise KeyError(f"recipient account frozen: {to_aid}")
        if amount <= 0: raise KeyError("amount must be positive")
        try:
