...
class OvalMCPWorkerContext:
    ...
    def create_session(
        ...
        session_kwargs: dict[str, Any] = {"seed": seed}
        if state_profiles is not None:
            session_kwargs["state_profiles"] = state_profiles
        if server_names is not None:
            session_kwargs["server_names"] = server_names
        session = self.manager.create_session(**session_kwargs)
        try:
