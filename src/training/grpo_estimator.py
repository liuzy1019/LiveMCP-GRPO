...
只从 non_tensor_batch（由 _get_gen_batch 保留，estimator.py 转发）读取
...
def _resolve_lata_mode(config) -> str:
    ...
    # 1) hydra config（如果已通过 estimator 注入）
    if config:
        # VERL passes a non-empty algorithm config in real training, but that
        # config does not normally contain our extension key.  Only treat the
        # Hydra config as authoritative when it explicitly declares LATA;
        # otherwise continue to the canonical LiveMCP environment config.
        raw = config.get("lata_mode", config.get("lata"))
        if raw is not None:
            val = str(raw).lower()
...
def _action_signature(raw_events) -> str:
    """Return a canonical signature for one rollout's functional actions."""
    if isinstance(raw_events, np.ndarray):
        raw_events = raw_events.tolist()
    ...
    if not isinstance(raw_events, list):
        return f"invalid-{type(raw_events).__name__}"
    actions = []
    for event in raw_events:
        if not isinstance(event, dict):
            actions.append(["invalid-event", type(event).__name__])
            continue
        actions.append(
            [
...
def compute_livemcp_grpo_advantage(
    ...
    **kwargs,
    ...
    - non_tensor_batch 通过 estimator.py 的 monkey-patch 注入
    ...
    bsz = scores.shape[0]
    if raw_j is None:
        raise ValueError("livemcp_grpo requires raw_j for saturation diagnostics")
    raw_j = raw_j.to(device=scores.device, dtype=scores.dtype)
    if raw_j.ndim != 1 or raw_j.shape[0] != bsz:
        raise ValueError(
            f"livemcp_grpo raw_j shape mismatch: {tuple(raw_j.shape)} vs ({bsz},)"
        )
    if not torch.isfinite(raw_j).all():
        raise ValueError("livemcp_grpo raw_j contains non-finite values")
    ...
    with torch.no_grad():
        ...
        # 饱和组 rollout 仍参与 lambda_safe 的 hat_C_batch（在 estimator 层已处理）
        min_group_std = float(
        ...
        audit_events = _to_list(non_tensor_batch.get("audit_events", []))
        has_action_diagnostics = len(audit_events) == bsz
        ...
        if max_group_size >= 2:
            for tid, indices in task2indices.items():
                if len(indices) < 2:
                    continue
                group_index = torch.tensor(indices, device=scores.device)
                group_j = raw_j[group_index]
                group_post_kl = scores[group_index]
                g_mean = group_j.mean()
                g_var = ((group_j - g_mean) ** 2).mean()
                g_std = g_var.sqrt()
                post_kl_std = (
                    (group_post_kl - group_post_kl.mean()).square().mean().sqrt()
                )
                unique_actions = (
                    len({_action_signature(audit_events[i]) for i in indices})
                    if has_action_diagnostics
                    else -1
                )
                group_diagnostics.append(
                    f"{tid}:raw_std={g_std.item():.3e},"
                    f"post_kl_std={post_kl_std.item():.3e},"
                    f"unique_actions={unique_actions}/{len(indices)}"
                )
                if g_std < min_group_std:
                    advantages[group_index] = 0.0
                    n_saturated_groups += 1
                    saturated_group_ids.append(tid)

        logger.info(
            "[livemcp_grpo] group diagnostics: "
            + " | ".join(group_diagnostics)
        )
        if n_saturated_groups > 0:
            logger.warning(
                f"[livemcp_grpo] SATURATION: {n_saturated_groups}/{n_total_groups} groups skipped "
                f"(raw std(J) < {min_group_std:.0e}). "
