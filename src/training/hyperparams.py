...
class LiveMCPHyperparams:
    ...
    # Per-run shared state path. The launcher resolves this into the
    # experiment directory to prevent cross-run contamination.
    lambda_state_path: str = "/tmp/ovalmcp_lambda_state.json"
    ...
    # Explicit XML terminal actions are the strict default. Plain final-answer
    # inference is an opt-in local compatibility mode.
    plain_final_compat: bool = False
    ...
    _ENV_MAP: ClassVar[dict[str, str]] = {
        ...
        "lambda_state_path": "OVAL_LAMBDA_STATE_PATH",
        ...
        "plain_final_compat": "OVAL_PLAIN_FINAL_COMPAT",
    ...
    def __post_init__(self) -> None:
        ...
        if 0 < self.max_observation_chars < 256:
            raise ValueError("max_observation_chars must be 0 or >= 256")
        if not self.lambda_state_path:
            raise ValueError("lambda_state_path must be non-empty")
    ...
    def summary(self) -> str:
        ...
        sections = {
            ...
            "Safety": ["lambda_safe_default", "lambda_state_path", "alpha_lambda", "lambda_epsilon", "lambda_safe_max", "k_stall", "tau_unsafe_stall"],
            "Advantage": ["min_group_std"],
            "LATA": ["lata_mode"],
            "TaskReward": ["w_val", "w_cov", "w_eff", "w_name", "w_arg", "alpha_eff", "beta_budget"],
            "P_process": ["p_max"],
            "Loop": ["suite_path", "domains", "max_observation_chars", "plain_final_compat"],
    ...
    def resolve_max_observation_chars(self, suite_rollout: dict) -> int:
        """Resolve the observation budget identically in every process."""
        configured = int(self.max_observation_chars)
        if configured <= 0:
            configured = int(suite_rollout.get("observation_max_chars", 4096))
        if configured < 256:
            raise ValueError(
                "effective max_observation_chars must be at least 256"
            )
        return configured
