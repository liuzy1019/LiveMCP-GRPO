...
def register_livemcp_estimator(config: Optional[dict] = None) -> bool:
    ...
    try:
        importlib.import_module("src.training.grpo_estimator")
    ...
    try:
        ...
        def patched_compute_advantage(data, adv_estimator, *args, **kwargs):
            ...
            if str(adv_estimator) == "livemcp_grpo":
                ...
                if "j" not in non_tensor_batch:
                    raise ValueError(
                        "livemcp_grpo requires raw task reward field 'j'"
                    )
                try:
                    raw_j = torch.as_tensor(
                        np.asarray(non_tensor_batch["j"], dtype=np.float32),
                        device=data.batch["token_level_rewards"].device,
                        dtype=data.batch["token_level_rewards"].dtype,
                    )
                except (TypeError, ValueError) as exc:
                    raise ValueError(
                        "livemcp_grpo raw task reward field 'j' is not numeric"
                    ) from exc
                if raw_j.ndim != 1 or raw_j.shape[0] != bsz:
                    raise ValueError(
                        "livemcp_grpo raw task reward length mismatch: "
                        f"shape={tuple(raw_j.shape)}, batch_size={bsz}"
                    )
                if not torch.isfinite(raw_j).all():
                    raise ValueError("livemcp_grpo raw task reward contains non-finite values")
                adv_kwargs["raw_j"] = raw_j
