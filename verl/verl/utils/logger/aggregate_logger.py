...
# compact 模式下按组显示的关键指标
_COMPACT_METRIC_GROUPS = {
    "train": [
...
# 向后兼容：展平为 tuple
_COMPACT_METRIC_KEYS = tuple(
    key for keys in _COMPACT_METRIC_GROUPS.values() for key in keys
)
...
def _short_key(key: str) -> str:
    """缩短 metric key 用于终端显示。"""
    # training/global_step -> step, actor/pg_loss -> pg_loss
    parts = key.rsplit("/", 1)
    return parts[-1] if len(parts) > 1 else key


def concat_dict_to_str(dict: dict, step):
    mode = os.environ.get("LIVEMCP_CONSOLE_LOG_MODE", "full").lower()

    if mode == "compact":
        # 单行精简格式：step | loss=x reward=x kl=x | t/s=x tput=x
        parts = [f"\033[1mstep {step}\033[0m"]
        for group_name, keys in _COMPACT_METRIC_GROUPS.items():
            group_items = []
            for key in keys:
                if key in dict and isinstance(dict[key], numbers.Number):
                    group_items.append(f"{_short_key(key)}={_format_metric_value(dict[key])}")
            if group_items:
                parts.append(" ".join(group_items))
        # val-core/ 指标
        val_items = []
        for key, value in dict.items():
            if key.startswith("val-core/") and isinstance(value, numbers.Number):
                val_items.append(f"{_short_key(key)}={_format_metric_value(value)}")
        if val_items:
            parts.append(" ".join(val_items))
        return " │ ".join(parts)
...
class LocalLogger:
    ...
    def log(self, data, step):
        if self.print_to_console:
            msg = concat_dict_to_str(data, step=step)
            # 如果有活跃的 tqdm 进度条，用 tqdm.write 避免打断
            try:
