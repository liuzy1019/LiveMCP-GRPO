...
class RayPPOTrainer:
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        # LiveMCP-GRPO 需要这些字段在 estimator 中计算分层 advantage，
        # 不能从 batch.non_tensor_batch 移除。
        schemashift_persist_keys = {
            "episode_id",
            "group_id",
            "task_id",
            "perturbation_level",
        ...
        reward_model_keys |= schemashift_persist_keys & batch.non_tensor_batch.keys()
        ...
        # 将 schemashift 字段也复制到 gen_batch，供 agent loop 使用
        for _key in schemashift_persist_keys:
            if _key in batch.non_tensor_batch and _key not in gen_batch.non_tensor_batch:
                gen_batch.non_tensor_batch[_key] = batch.non_tensor_batch[_key]
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if os.environ.get("LIVEMCP_VERBOSE_VALIDATION", "0") == "1":
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # tqdm postfix: 关键 metrics 一目了然
                _postfix = {}
                if "actor/pg_loss" in metrics:
                    _postfix["loss"] = f"{metrics['actor/pg_loss']:.4f}"
                if "critic/score/mean" in metrics:
                    _postfix["reward"] = f"{metrics['critic/score/mean']:.3f}"
                if "actor/ppo_kl" in metrics:
                    _postfix["kl"] = f"{metrics['actor/ppo_kl']:.4f}"
                if "perf/time_per_step" in metrics:
                    _postfix["t/s"] = f"{metrics['perf/time_per_step']:.1f}s"
                if _postfix:
                    progress_bar.set_postfix(_postfix, refresh=False)
