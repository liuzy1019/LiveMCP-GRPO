...
class DataAuditor:
    ...
    def check_scenario_coverage(self) -> bool:
        ...
        required = {"task_planner", "missing_function", "irrelevant", "distractor"}
        missing = required - val_scenarios
        ok = len(missing) == 0
        if not ok:
            self.issues.append(f"Val missing scenarios: {missing}")
        return ok

    def check_missing_function_oracle_empty(self) -> bool:
        """missing_function 类型的 oracle_calls 必须为空列表。"""
        violations = []
        for _, r in self.all_data.iterrows():
            ei = self._pe(r["extra_info"])
        ...
        ok = len(violations) == 0
        if not ok:
            self.issues.append(f"missing_function with non-empty oracle: {violations[:5]}")
        return ok

    def check_irrelevant_oracle_empty(self) -> bool:
        """irrelevant 类型的 oracle_calls 必须为空列表。"""
        violations = []
        for _, r in self.all_data.iterrows():
            ei = self._pe(r["extra_info"])
        ...
        ok = len(violations) == 0
        if not ok:
            self.issues.append(f"irrelevant with non-empty oracle: {violations[:5]}")
        return ok
    ...
    def check_has_missing_function_consistency(self) -> bool:
        ...
        for _, r in self.all_data.iterrows():
            ...
            if st == "missing_function" and not hmf:
                violations.append(ei.get("task_id", "?"))
            if st != "missing_function" and hmf:
                violations.append(ei.get("task_id", "?"))
        ok = len(violations) == 0
        if not ok:
    ...
    def check_domain_coverage(self) -> bool:
        """验证多域覆盖。"""
        domains = Counter()
        for _, r in self.all_data.iterrows():
            ei = self._pe(r["extra_info"])
        ...
        ok = len(domains) >= 2
        if not ok:
            self.issues.append(f"Only {len(domains)} domain(s): {dict(domains)}")
        return ok
