...
覆盖:
  A. Server TOOLS annotation 与 _MUTATING_PREFIXES / _SELF_CONTAINED / 前缀的全量匹配
  B. _TOOL_ENTITY_OVERRIDE 在两文件间的一致性
...
from src.live_mcp.orchestrator import (
    ...
    _UNSAFE_SHORTCUT_TOOLS, _TOOL_ENTITY_OVERRIDE as orch_override,
...
from src.live_mcp.task_planner import (
    _is_mutating_tool, _MUTATING_PREFIXES, _SELF_CONTAINED_WRITE_TOOLS,
)
                    ...
                    for k, v in zip(elt.keys, elt.values):
                        if isinstance(k, ast.Constant):
                            if isinstance(v, ast.Constant):
                                ...
                                d[k.value] = inner
                    name = d.get("name")
                    ann_raw = d.get("annotations") or {}
                    if isinstance(ann_raw, str):  # malformed
                        ann_raw = {}
                    if name:
                        tools.add(name)
                        anns[name] = {
                            "mutating": bool(ann_raw.get("mutating", False)),
                            "readonly": bool(ann_raw.get("readonly", False)),
                            "sensitive_params": bool(ann_raw.get("sensitive_params", False)),
                        }
...
for domain, anns in domain_anns.items():
    for name, ann in anns.items():
        if ann["mutating"] and not ann["readonly"]:
            all_mutating.add(name)

# ═════════════════════════════════════════════════════════════════
# A. _MUTATING_PREFIXES 100% coverage of all server-annotated mutating tools
# ═════════════════════════════════════════════════════════════════
section("A. _MUTATING_PREFIXES vs 所有 server-annotated mutating tools")

missed_mutating = []
for tool in sorted(all_mutating):
    if not _is_mutating_tool(tool):
        missed_mutating.append(tool)

if missed_mutating:
    for t in missed_mutating:
        print(f"  ❌ {t} is annotated mutating but _is_mutating_tool returns False")
    FAIL += len(missed_mutating)
else:
    print(f"  ✅ All {len(all_mutating)} server-annotated mutating tools covered")

# ═════════════════════════════════════════════════════════════════
# A2. 反向检测：_MUTATING_PREFIXES 匹配到的 tool 是否真的存在
# ═════════════════════════════════════════════════════════════════
section("A2. _MUTATING_PREFIXES 是否匹配到了不存在的 ghost tool")

prefix_ghosts = 0
for prefix in _MUTATING_PREFIXES:
    matched = [t for t in all_tool_names if t.lower().startswith(prefix)]
    if not matched:
        print(f"  ⚠️  Prefix '{prefix}' matches NO existing tools (dead prefix)")
        prefix_ghosts += 1
if prefix_ghosts:
    print(f"  ⚠️  {prefix_ghosts} dead prefixes found (not failures, but worth cleanup)")
else:
    print(f"  ✅ All prefixes match at least one existing tool")

# ═════════════════════════════════════════════════════════════════
# B. _TOOL_ENTITY_OVERRIDE 两文件一致性
# ═════════════════════════════════════════════════════════════════
section("B. _TOOL_ENTITY_OVERRIDE orchestrator vs task_planner 一致性")

# Extract task_planner._tool_entity's _ov dict
from src.live_mcp.task_planner import _tool_entity as tp_entity_raw
import inspect, re
tp_src = inspect.getsource(tp_entity_raw)

# Parse _ov dict: handles multi-key-per-line like `"a": "x", "b": "y",`
tp_ov = {}
match = re.search(r'_ov:\s*dict\[str,\s*str\]\s*=\s*\{(.+?)\}', tp_src, re.DOTALL)
if match:
    body = match.group(1)
    for kv_match in re.finditer(r'"(\w+)"\s*:\s*"(\w+)"', body):
        tp_ov[kv_match.group(1)] = kv_match.group(2)

# Compare
all_keys = set(orch_override.keys()) | set(tp_ov.keys())
b_fail = False
for k in sorted(all_keys):
    ov = orch_override.get(k)
    tv = tp_ov.get(k)
    if ov != tv:
        print(f"  ❌ Key '{k}': orchestrator={ov}, task_planner={tv}")
        b_fail = True
if not b_fail:
    print(f"  ✅ All {len(orch_override)} overrides identical across files")
else:
    FAIL += 1
...
# ═════════════════════════════════════════════════════════════════
# D2. _DOMAIN_TOOL_REQUIREMENTS 中 mutating tool 的 entity 应匹配 _tool_entity
# ═════════════════════════════════════════════════════════════════
section("D2. _DOMAIN_TOOL_REQUIREMENTS entity vs _tool_entity 一致性")

d2_failures = 0
for domain, reqs in sorted(_DOMAIN_TOOL_REQUIREMENTS.items()):
    for tool, entities in sorted(reqs.items()):
        te = orch_entity(tool, domain)
        # For mutating tools, entity should be in requirements
        ann = domain_anns.get(domain, {}).get(tool, {})
        if ann.get("mutating") and te not in entities:
            # Check if there's a reasonable match
            print(f"  ⚠️  {domain}/{tool}: _tool_entity='{te}' but requirements={entities}")
            d2_failures += 1
if d2_failures:
    print(f"  ⚠️  {d2_failures} potential entity mismatches (not hard errors)")
else:
    print(f"  ✅ All entity mappings consistent with requirements")
...
for domain, anns in sorted(domain_anns.items()):
    domain_mutating = {t for t, a in anns.items()
                       if a["mutating"] and not a["readonly"]}
...
# ═════════════════════════════════════════════════════════════════
# G. _MUTATING_PREFIXES 前缀冲突检测
# ═════════════════════════════════════════════════════════════════
section("G. _MUTATING_PREFIXES 前缀冲突检测")

sorted_prefixes = sorted(_MUTATING_PREFIXES, key=len, reverse=True)
conflicts = []
for i, p1 in enumerate(sorted_prefixes):
    for j, p2 in enumerate(sorted_prefixes):
        if i >= j:
            continue
        # p1 is longer, p2 is shorter
        if p1.startswith(p2):
            conflicts.append((p2, p1))
if conflicts:
    for shorter, longer in conflicts:
        print(f"  ⚠️  Potential shadow: '{shorter}' is prefix of '{longer}' (longer always unreachable)")
else:
    print(f"  ✅ No prefix shadowing conflicts")
...
h_cases = [
    ...
    # Case 9: domain boundary mismatch → executor tool not from server_name
    ("wrong_domain_OK",
     "shopping", [("send_email", {})], False,
     "send_email has no shopping existing-entity requirements"),
    # Case 10: forward_email with list_inbox → OK
    ("forward_OK",
     "email", [("list_inbox", {}), ("forward_email", {})], False,
     "list_inbox→forward_email should pass"),
    # Case 11: mark_read with list_inbox → OK
    ("mark_read_OK",
     "email", [("list_inbox", {}), ("mark_read", {})], False,
     "list_inbox→mark_read should pass"),
    # Case 12: update_event alone (calendar executor) → flag
    ("update_event_alone_BAD",
     "calendar", [("update_event", {})], True,
     "update_event without preceding read → missing dep"),
    # Case 13: create_event (creator) → exempted
    ("create_event_OK",
...
for name, domain, calls_raw, expect, desc in h_cases:
    ...
    if result != expect:
        ...
        h_fail = True
if not h_fail:
    print(f"  ✅ All {len(h_cases)} _detect_missing_dependency edge cases pass")
...
# ═════════════════════════════════════════════════════════════════
# J. _tool_entity 两文件全量对比（所有 server tool）
# ═════════════════════════════════════════════════════════════════
section("J. _tool_entity 全量对比（所有 server tool）")

j_fail = False
for domain, tools in sorted(domain_tools.items()):
    for tool in sorted(tools):
        oe = orch_entity(tool, domain)
        te = tp_entity_raw(tool, domain)
        if oe != te:
            print(f"  ❌ {domain}/{tool}: orchestrator='{oe}', task_planner='{te}'")
            j_fail = True
if not j_fail:
    print(f"  ✅ All domain/tool entity mappings are identical in both files")
else:
    FAIL += 1
...
total_mutating = len(all_mutating)
total_domains = len(domain_tools)
print(f"  Domains: {total_domains}")
print(f"  Total tools: {total_tools}")
print(f"  Mutating tools: {total_mutating}")
